...
class ConnectionOptions {
    ...
    void check()  {
        ...
        if (events.size() != peers.size()) {
            throw new IllegalArgumentException("there should be an identical number of event hub urls to peers");
        }
    ...
    private static void checkField(Object field, String name) {
        if (field == null) {
            throw new IllegalArgumentException("The " + name + " has not been specified in the connection profile");
        }
    }

    private static void checkListField(List field, String name) {
        checkField(field, name);
        if (field.isEmpty()) {
            throw new IllegalArgumentException("No " + name + " have been specified in the connection profile");
        }
    }
